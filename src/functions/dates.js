...
function sortDates(projectTitle, today, projectDueDate, projectYear, projectMonth, projectDay, todaysYear, todaysMonth, todaysDay, projectPriority) {
    let projectTitleId;
    ...
    if (projectDueDate == today) {
        let dueStatus = 'today';
        createDueCard(projectTitleId, projectDueDate, projectTitle, dueStatus, projectPriority);
    } else if (projectDueDate != today && projectMonth == todaysMonth && _.inRange(projectDay, today += 1, todaysDay += 7)) {
        let dueStatus = 'this week';
        createDueCard(projectTitleId, projectDueDate, projectTitle, dueStatus, projectPriority);
    } else if (projectDueDate != today && projectMonth == todaysMonth) {
        let dueStatus = 'this month';
        createDueCard(projectTitleId, projectDueDate, projectTitle, dueStatus, projectPriority);
    }
}

function createDueCard(projectTitleId, projectDueDate, projectTitle, dueStatus, projectPriority) {
